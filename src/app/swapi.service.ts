...
const mapRequestDto = (appRequest: AppModels.ListRequest) => {
  const nameFilter = appRequest.filters && appRequest.filters.name;
  return {
    page: nameFilter ? "1" : appRequest.page.toString(),
    search: nameFilter || ""
  };
};

// Response: Map dto model to app domain model
const mapResponseDto = <T>(mapItem: (item: any) => T) => (
  response: ListResponseDTO
): AppModels.ListResponse<T> => {
  return {
    totalCount: response.count,
    page: response.next
      ? parseInt(response.next.split("page=")[1]) - 1
      : (response.previous
      ? parseInt(response.previous.split("page=")[1]) + 1
      : 1),
